...
test("Convert shorthand URL", async (t) => {
  ...
  t.is(
    await getAuthUrl({
      ...
      env,
      branch: { name: "master" },
      options: { repositoryUrl: "nholuongut/semantic-release" },
    }),
    "https://github.com/nholuongut/semantic-release.git"
  );
});
...
test("Convert GitLab shorthand URL", async (t) => {
  ...
  t.is(
    await getAuthUrl({
      ...
      env,
      branch: { name: "master" },
      options: { repositoryUrl: "gitlab:nholuongut/semantic-release" },
    }),
    "https://gitlab.com/nholuongut/semantic-release.git"
  );
});
